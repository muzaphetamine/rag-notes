...
def extract_question_banks(input_dir: Path, output_dir: Path) -> None:
    ...
    for i, file_path in enumerate(files, start=1):
        ...
        try:
            ...
            # Docling's structured export preserves tables and
            # OCR'd content much better than plain PDF extraction.
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract study material and question banks using Docling"
    )
    ...
    parser.add_argument(
        ...
        help="Folder containing study material"
    )

    parser.add_argument(
        "--output",
        type=str,
        default="output/docjson",
        help="Folder to write study-material Docling JSON"
    )

    parser.add_argument(
        "--questions",
        type=str,
        default="input/questions",
        help="Folder containing question banks"
    )
    ...
    if not question_dir.exists():
        raise FileNotFoundError(
            f"Question folder not found: {question_dir}"
        )
